...
class AddEditBoyViewController: FormViewController {
    ...
    override func viewDidLoad() {
        ...
        // because its editing we haave a name of boy to display
     ...
     form +++ Section("Boys Name")
       ...
       self.selectedNasiBoy?.boyLastName = row.value ??
        ...
        form +++ Section("Boys Age")
        <<< PhoneRow() {
            
            $0.placeholder = "Enter Boys Age"
            
            // check if we are editing or adding
            // if we are adding then age will be zero
            // if we are editing then age may be empty
            // or we may be able to calculate the age based
            // on the date of birthj
            // so we need to calculate the age based on the dobString
            let dobIntervalString = selectedNasiBoy.dobIntervalString ?? ""
            
            let boyAge = self.selectedNasiBoy.calculateAgeFrom(dobString: dobIntervalString)
            $0.value = "\(boyAge)"
            $0.onChange { [unowned self] row in
    self.selectedNasiBoy?.ageEntered = Int(row.value ?? "0") ?? 0
    //convert age to dob
    // take the age entered and use it to to calculate date of birth
    let dob = dobByAddingYears(numberOfYears: self.selectedNasiBoy.ageEntered)
    
    // create a dateFormatt4er to convert from Date to string
    let dateFormatter = DateFormatter()
    dateFormatter.dateFormat = "YY/MM/dd"
    let dobString = dateFormatter.string(from: dob)
                
    // now we store that string
    self.selectedNasiBoy.dobIntervalString = dobString
        ...
        <<< TextRow() {
           $0.placeholder = "City"
            $0.value = selectedNasiBoy?.city ?? ""
          $0.onChange { [unowned self] row in //6
  self.selectedNasiBoy?.city = row.value ?? ""
          }
      }
    ...
    form  +++ Section()
            ...
            if selectedNasiBoy != nil {
               ...
               let date = dateFormatter.date(from: dobString)
                ...
                // get the date as a date from the picker
                // convert to a string date
                ...
                // pass in birthday as a date and get the timeInterval
                // as an Int
...
form
    ...
    <<< ActionSheetRow<String>() {
       ...
       <<< PhoneRow() {
        ...
        <<< TextAreaRow() {
            $0.value = selectedNasiBoy.shadchanNotesNew
            
            $0.textAreaHeight = .dynamic(initialTextViewHeight: 110)
            
            $0.onChange { [unowned self] row in //6
                self.selectedNasiBoy?.shadchanNotesNew = row.value ?? ""
            }
    }
    ...
    func fetchDatingHistoryForThisBoyUsingKey(key: String)  {
...
//                    do nothing
    ...
    // then  we will have stored it in the key dobIntervalString
    // and now we need to convert it into an age string
    // to present ot the user
    // but we first check to see that it isnt empty which
    // can be because no age was entered or because
    // we are addidng a brand new boy
    func checkAndCalculateAgeFromDobInterval(String: String) -> String {
        var ageString = ""
        
        // we dont have to worry about selectedNasiBoy being nil
        // because if we arent editing we still init a new nasiboy
        // but it may still be empty if we init a new object
        // of if the user never enetered an age
        // so either way we want to return an empty string
        if self.selectedNasiBoy!.dobIntervalString == "" {
            // return an empty string
            return ageString
         } else { // if we have  not only a non nil shadchan boy
             // but actually have a dobInterval string
             // then use that string to calculate age
             
             let age = calculateAgeFrpm(dobString: selectedNasiBoy.dobIntervalString)
             let ageString = "\(age)"
